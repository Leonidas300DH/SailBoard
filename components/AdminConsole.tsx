...
function Fleet({ snapshot, send, busy }: { snapshot: Snapshot; send: (url: string, payload: unknown) => Promise<boolean>; busy: boolean }) {
  ...
  return <div className="admin-grid"><article className="admin-panel"><div className="admin-panel-head"><h2>Ajouter un bateau</h2></div><form className="admin-panel-body form-grid" onSubmit={(event) => void onSubmit(event, "createBoat")}><FormField label="Nom"><input name="name" required /></FormField><FormField label="N° de voile"><input name="sailNumber" required /></FormField><FormField label="Modèle"><input name="model" required defaultValue="J/80" /></FormField><FormField label="Couleur"><input name="color" type="color" defaultValue="#d9ff00" /></FormField><div className="full"><button className="button primary" disabled={busy}><Plus />Ajouter</button></div></form></article>
    <article className="admin-panel"><div className="admin-panel-head"><h2>Ajouter un navigateur</h2></div><form className="admin-panel-body form-grid" onSubmit={(event) => void onSubmit(event, "createParticipant")}><FormField label="Nom complet" full><input name="name" required /></FormField><FormField label="Nationalité"><input name="nationality" defaultValue="FR" maxLength={2} /></FormField><FormField label="Profil public"><select name="publicVisible" defaultValue="1"><option value="1">Visible</option><option value="0">Masqué</option></select></FormField><div className="full"><button className="button primary" disabled={busy}><Plus />Ajouter</button></div></form></article>
    <article className="admin-panel full"><div className="admin-panel-head"><h2>Engager un bateau et son équipage</h2></div><form className="admin-panel-body form-grid" onSubmit={(event) => void onSubmit(event, "createEntry")}><FormField label="Manche"><select name="raceId" required>{snapshot.races.map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.name)}</option>)}</select></FormField><FormField label="Bateau"><select name="boatId" required>{snapshot.boats.map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.name)}</option>)}</select></FormField><FormField label="Barreur"><select name="helmId" required>{snapshot.participants.map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.name)}</option>)}</select></FormField><FormField label="Régleur"><select name="trimId" required>{snapshot.participants.map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.name)}</option>)}</select></FormField><FormField label="Avant"><select name="bowId" required>{snapshot.participants.map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.name)}</option>)}</select></FormField><FormField label="N° de départ"><input name="startNumber" type="number" min="1" required /></FormField><div className="full"><button className="button primary" disabled={busy}><Anchor />Engager l’équipage</button></div></form></article>
    ...
    <article className="admin-panel"><div className="admin-panel-head"><h2>Navigateurs</h2><span>{snapshot.participants.length}</span></div><div className="admin-panel-body admin-list">{snapshot.participants.slice(0, 12).map((item) => <div className="admin-list-row" key={String(item.id)}><div><strong>{String(item.name)}</strong><small>{item.public_visible ? "Profil public" : "Profil masqué"}</small></div><Users size={16} /></div>)}</div></article></div>;
}
